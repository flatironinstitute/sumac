    ...
    int N, int M)
{
  ...
  __shared__ float4 As4[BK][D/4];

  const int tid  = threadIdx.x;

  const int m  =  blockIdx.x * blocksize + tid;
  ...
  if (m < M) {
    const float4* __restrict__ B4 =
        reinterpret_cast<const float4*>(__builtin_assume_aligned(B, 16));
  ...
  for (int n0 = 0; n0 < N; n0 += BK) {
    ...
    for (int idx = tid; idx < BK * (D/4); idx += blocksize) {
      int k  = idx / (D/4);
      int dv = idx - k * (D/4);
      int n  = n0 + k;
      float4 v;
    ...
    for (int k = 0; k < BK; ++k) {
      ...
      // Preload next (k+1) early, except on last iter
  ...
  if (m < M) {
    float4* Y4 = reinterpret_cast<float4*>(__builtin_assume_aligned(Y, 16));
    const int row4 = m * (D / 4);
    Y4[row4 + 0] = y0;
    Y4[row4 + 1] = y1;
    Y4[row4 + 2] = y2;
...
torch::Tensor relu_bat_a_fused_cuda(torch::Tensor A, torch::Tensor B) {
  ...
  relu_bat_a_fused_kernel<<<grid, block, 0>>>(
