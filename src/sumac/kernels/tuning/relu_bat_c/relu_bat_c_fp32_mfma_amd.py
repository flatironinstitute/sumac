...
def relu_bat_c_fp32_mfma_tune_config(
    D: int,
) -> list[ReluBatCFp32MfmaTuneConfig]:
    if D >= 256:
        template = ReluBatCFp32MfmaTuneConfig(BM=16, BN=16, M_TILES=1)
        choices = {
            "BM": [16, 32, 64],
            "BN": [16, 32],
            "M_TILES": [1, 2],
        ...
        template = ReluBatCFp32MfmaTuneConfig(BM=32, BN=16, M_TILES=1)
        choices = {
            "BM": [32, 64, 128],
            "BN": [16, 32, 64],
            "M_TILES": [1, 2, 4],
        ...
        template = ReluBatCFp32MfmaTuneConfig(BM=64, BN=16, M_TILES=1)
        choices = {
            "BM": [64, 128, 256],
            "BN": [16, 32, 64, 128],
            "M_TILES": [1, 2, 4],
...
class AutotuneReluBatCFP32MfmaAMD(
    AutotuneCudaKernel[
        ReluBatCFp32MfmaTuneConfig,
        T_ReluBatCParams,
        T_ReluBatCReturn,
    ...
    def __init__(self, *args, **kwargs):
        kwargs["wrapped_fn_name"] = relu_bat_c_fp32_mfma.__name__
        kwargs["wrapped_fn_module"] = relu_bat_c_fp32_mfma.__module__
        super().__init__(*args, **kwargs)

    def _candidate_fn(
        self,
        params: T_ReluBatCParams,
        config: ReluBatCFp32MfmaTuneConfig,
    ):
        A, B, C = params
        return relu_bat_c_fp32_mfma(
            A,
            B,
            C,
            BM=config.BM,
    ...
    def _constraint(
        ...
        A, _, _ = params
        if not relu_bat_c_fp32_mfma_available(A.device):
            return False
