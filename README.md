...
$$
S = \operatorname{ReLU}(A B^{\top}), \quad A, B \in \mathbb R^{m \times d}, d \ll \min(m,n) 
$$

We offer two major routines:
- Alternating Least Sqaures (sumac-ALS): update factors $A,B$ alternatively by solving least squares, together with an auxiliary variable $Z$ where $\min_{Z, A, B} \| Z - A B^{\top} \|_F^2, S = \operatorname{ReLU}(Z)$
