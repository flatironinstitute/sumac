...
Common options are `method="SALSA"` or `method="GD"`, `rank`, `max_iterations`, `num_blocks`, `seed`, `momentum`, `learning_rate`, `optimizer`, `eval_interval`, `verbose`, and `allow_tf32`. The dtype of `S_value` determines the computation dtype and must be either `torch.float32` or `torch.float64`. CUDA kernel autotuning can be controlled with `autotune="cache"`, `"force"`, `"disable"`, or `"fallback"`; cached tuning results are stored under `$XDG_CACHE_HOME/sumac` by default, or `~/.cache/sumac` if `XDG_CACHE_HOME` is unset. Use `autotune_cache_dir` to override this location and `autotune_verbose=True` to print tuning decisions.
